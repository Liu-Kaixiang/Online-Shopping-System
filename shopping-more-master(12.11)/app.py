...
def getLoginDetails():
    with getConnection() as conn:
        ...
        if 'email' not in session:
            ...
            cur.execute("SELECT count(order_id) FROM goods_order WHERE user_id = ?", (userId, ))
...
def goods(goods_id):
    ...
    if len(data) > 3:
        data = data[0:3]
    return render_template("goods.html", categoryData=categoryData, itemData=itemData, data=data, shopData=shopData)
...
def test():
    ...
    with getConnection() as conn:
        cur = conn.cursor()
        cur.execute('SELECT shop_id, shop_name, address, phone, shop_describe, announcement FROM shop WHERE shop_id=?', (shop_id,))
        itemData = cur.fetchall()[0]
        #print(itemData)
        ...
        # cur.execute('SELECT shop.shop_id, shop_name FROM shop, goods WHERE goods.goods_id=? AND goods.shop_id=shop.shop_id', (goods_id,))
        # shopData = cur.fetchall()[0]
    return render_template("store.html", itemData=itemData, goodsData=goodsData)
...
def store(shop_id):
    ...
    with getConnection() as conn:
        cur = conn.cursor()
        cur.execute('SELECT shop_id, shop_name, address, phone, shop_describe, announcement', (shop_id,))
        itemData = cur.fetchall()[0]
        print(itemData)
        cur.execute('SELECT goods.goods_id, goods_name, price, goods_describe, image_addr, inventory FROM goods, goods_image, goods_attribute WHERE goods.goods_id=goods_image.goods_id AND goods.goods_id=goods_attribute.goods_id AND goods.shop_id=?', (shop_id,))
        categoryData = cur.fetchall()[0]
        # cur.execute('SELECT shop.shop_id, shop_name FROM shop, goods WHERE goods.goods_id=? AND goods.shop_id=shop.shop_id', (goods_id,))
        # shopData = cur.fetchall()[0]
    return render_template("store.html", itemData=itemData, categoryData=categoryData)
...
def addToCart():
    ...
    with getConnection() as conn:
        ...
        cur.execute("SELECT COUNT(*) FROM kart WHERE user_id = ? AND goods_id = ?", (userId, productId))
        num = cur.fetchall()
        print(num)
        try:
            if len(num) > 0:
                ...
                cur.execute("INSERT INTO goods_order (user_id, goods_id, num) VALUES (?, ?, ?)", (userId, productId, 1))
            conn.commit()
...
def cart():
    ...
    with getConnection() as conn:
        ...
        userId = cur.fetchone()[0]
        cur.execute("SELECT products.productId, products.name, products.price, products.image, kart.num FROM products, kart WHERE products.productId = kart.productId AND goods_order.user_id = ?", (userId, ))
...
def removeFromCart():
    ...
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        cur.execute("SELECT userId FROM user_inf WHERE email = ?", (email, ))
        userId = cur.fetchone()[0]
