...
-- 查询每个省份会员的消费情况的视图
create view shopping_view
as
select user_inf.user_id, user_inf.province, sum(goods_num*actual_price) as consumption
from user_inf,goods_in_orders
where user_inf.user_id = goods_in_orders.user_id
group by user_inf.user_id,user_inf.province 
