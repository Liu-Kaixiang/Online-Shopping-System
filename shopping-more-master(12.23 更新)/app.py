...
def newOrder():
    ...
    with getConnection() as conn:
        ...
        try:
            cur.execute(
                "insert into goods_in_orders(goods_id,goods_num,cost,actual_price,shop_id,user_id) values(?,?,?,?,?,?)",
                (goods_id, num, cost,price, shop_id, user_id))
    ...
    if request.method == "POST":
        ...
        with getConnection() as conn:
            try:
                ...
                print(('###################SQL3############'))
                cur.execute("INSERT INTO goods_image(goods_id,image_addr,is_valid) VALUES(?,?,?)",(int(total)+1,imagename,1) )
                conn.commit()
                flash("added successfully")
            except:
                flash("error occured")
